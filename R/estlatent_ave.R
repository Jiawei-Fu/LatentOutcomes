#' Estimate the Causal Effects with Latent Outcomes (average-proxy GMM)
#'
#' @param mod Required regression specification for latent regression.
#'   Recommended form is RHS-only, e.g. `"~trt+x1+x2"`. Also accepts `"Y~..."` or `"eta~..."`.
...
#' @param data Optional data frame used to resolve character inputs in `Y`,
...
#'   Names can come from `data` (when provided), outcomes, and/or covariates implied by `mod`.
#'   Default is all regressors implied by `mod`.
#' @param opt Logical. For `method="gmm"`, `TRUE` (default) uses efficient two-step GMM;
#'   `FALSE` uses one-step GMM with identity weighting.
#'@examples
#' \dontrun{
#' data("test_data", package = "LatentOutcomes")
#' m1 <- estlatent_ave(mod = "~trt", Y = c("Y1","Y2","Y3"), data = test_dat, method = "sem")
#' summary(m1)
#' m2 <- estlatent_ave(mod = "~trt+x1+x2", Y = c("Y1","Y2","Y3"), data = test_dat,
#'                    method = "gmm", IV_Y = c("trt","Y2"), opt = TRUE)
#' summary(m2)
#'}
...
#'@export
estlatent_ave <- function(mod, Y, data = NULL, method = "sem", IV_Y = NULL, opt = TRUE) {
  ...
  if (method == "gmm") {
    ...
    n_reg_mom <- 1 + n_w
    n_mom <- n_load_mom + (n_y - 1) + n_y + n_reg_mom
    n_par <- 2 * n_y + 1 + n_w
    if (n_mom < n_par) {
      stop("gmm_ave under-identified: add more IVs via IV_Y")
    }

    g_opt <- function(theta, dat_gmm) {
      gmm_ave(theta, dat_gmm,
              mod = mod,
              n_y = n_y,
              w_idx = w_idx,
    ...
    final_lambda_est <- c(1, cf[2:n_y])
    final_lambda_se <- c(0, se[2:n_y])
  ...
  coef_name <- c(lambda_name, w_names)
